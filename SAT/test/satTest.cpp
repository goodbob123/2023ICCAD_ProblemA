...
void test1() {
   ...
   miterSolver.addAigCNF(f[0].getVar(), x[0].getVar(), false, x[1].getVar(), false);
   miterSolver.addAigCNF(f[1].getVar(), x[0].getVar(), true, x[1].getVar(), true);
   ...
   temp = miterSolver.newVar();
   g.push_back(Port("!a & b", temp));
   miterSolver.addAigCNF(g[0].getVar(), y[0].getVar(), false, y[1].getVar(), true);
   miterSolver.addAigCNF(g[1].getVar(), y[0].getVar(), true, y[1].getVar(), false);
}
...
void solve() {
   ...
   while (1) {
      ...
      if (!miterResult) {
         ...
         for (int i = 0; i < fStar.size(); ++i) {
            for (int j = 0; j < f.size(); ++j) {
               if (miterSolver.getValue(g[i].getVar()) != miterSolver.getValue(f[j].getVar())) { 
                  lits.push_back(~Lit(c[i][j].matrixVar));
               }
               else {
                  lits.push_back(~Lit(d[i][j].matrixVar));
               ...
               for (int k = 0; k < y.size(); ++k) {
                  for (int l = 0; l < x.size(); ++l) { // +1 or not
                     if (miterSolver.getValue(y[k].getVar()) != miterSolver.getValue(x[l].getVar())) {
